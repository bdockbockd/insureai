...
export function HeroSection() {
  ...
  return (
    <section className="relative min-h-screen overflow-hidden bg-gradient-to-br from-blue-50 via-white to-purple-50">
      {/* Animated Mesh Gradient Background - simplified on mobile for performance */}
      <div className="absolute inset-0 overflow-hidden">
        {/* Primary gradient orb - static on mobile, animated on desktop */}
        {isMobile ? (
          <div className="absolute -top-40 -right-40 w-[400px] h-[400px] bg-gradient-to-br from-blue-400/20 to-purple-400/20 rounded-full blur-2xl" />
        ) : (
          <motion.div
            animate={{
              scale: [1, 1.3, 1],
        ...
        {/* Secondary gradient orb - static on mobile */}
        {isMobile ? (
          <div className="absolute -bottom-40 -left-40 w-[400px] h-[400px] bg-gradient-to-br from-pink-400/20 to-orange-400/20 rounded-full blur-2xl" />
        ) : (
          <motion.div
            animate={{
              scale: [1.2, 1, 1.2],
        ...
        {/* Center gradient - static on mobile */}
        {isMobile ? (
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[500px] h-[500px] bg-gradient-to-r from-cyan-400/15 via-blue-400/15 to-purple-400/15 rounded-full blur-2xl" />
        ) : (
          <motion.div
            animate={{
              scale: [1, 1.1, 1],
      ...
      <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-20">
        {/* Trust Badge with glow effect - simplified on mobile */}
        <motion.div
        ...
        >
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-gradient-to-r from-blue-100 to-purple-100 text-blue-700 border border-blue-200/50 shadow-lg shadow-blue-500/20"
          >
            {isMobile ? (
        ...
        {/* Main Headline - simplified animations on mobile */}
        <motion.div
          initial={{ opacity: 0, y: isMobile ? 10 : 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.1, duration: isMobile ? 0.4 : 0.8, ease: "easeOut" }}
        ...
        >
          <h1 className="text-4xl sm:text-5xl lg:text-7xl font-bold text-gray-900 leading-tight mb-6">
            ...
            <span className="block relative">
              ...
              {/* Underline - static on mobile, animated on desktop */}
              {isMobile ? (
                <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-3/4 h-1.5 bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 rounded-full" />
              ) : (
                <motion.div
                  initial={{ scaleX: 0 }}
                  animate={{ scaleX: 1 }}
            ...
            <span className="text-3xl sm:text-4xl lg:text-5xl block mt-2">
              {t("hero.title3")}
            </span>
          </h1>
        ...
        {/* Insurance Type Cards - simplified on mobile */}
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
        ...
        >
          {insuranceTypes.map(({ icon: Icon, labelKey, color, shadowColor }, index) => (
            <motion.div
              ...
              animate={{ opacity: 1, y: 0 }}
              transition={{
                delay: isMobile ? 0.1 + index * 0.05 : 0.6 + index * 0.15,
              ...
              `}
            >
              {/* Icon - static on mobile, animated on desktop */}
              {isMobile ? (
        ...
        <motion.div
          initial={{ opacity: 0, y: isMobile ? 10 : 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: isMobile ? 0.3 : 1, duration: isMobile ? 0.3 : 0.6 }}
        ...
        >
          <Link href="/wizard" className="w-full sm:w-auto">
            <motion.div
              ...
              className="relative"
            >
              {/* Glow ring - static on mobile, animated on desktop */}
              {isMobile ? (
        ...
        <motion.div
          initial={{ opacity: 0, y: isMobile ? 10 : 30 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: isMobile ? 0.4 : 1.2, duration: isMobile ? 0.3 : 0.6 }}
          className="flex justify-center gap-10 sm:gap-20 lg:gap-32"
        >
          {stats.map(({ value, suffix, labelKey, icon: Icon, isDecimal }) => (
        ...
        {/* Floating CTA for Mobile - no animation for performance */}
        <motion.div
          initial={{ opacity: 0, y: 50 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.5, duration: 0.3 }}
          className="fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-xl border-t border-gray-200/50 sm:hidden z-40 shadow-lg shadow-blue-500/10"
        >
          <Button
            size="lg"
            variant="gradient"
